...
/**
 *stringuct passinfoo - contains pseudo-arggsuements to pass into a function,
...
void clear_infoo(inf_t *);
void set_infoo(inf_t *, char **);
void free_infoo(inf_t *, int);
...
char *get_hstry_file(inf_t *info);
int write_hstry(inf_t *info);
int read_hstry(inf_t *info);
int build_hstry_list(inf_t *info, char *buf, int linecount);
int renumberber_hstry(inf_t *info);
