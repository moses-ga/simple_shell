...
/**
 * createBuff - buffers chained commands
...
ssize_t createBuff(info_t *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			remove_comments(*buf);
			linkHistr(info, *buf, info->histcount++);
...
ssize_t get_input(info_t *info)
{
	...
	*buf_p = buf; /* else not a chain, pass back buffer from _getLines() */
	return (r); /* return length of buffer from _getLines() */
}
